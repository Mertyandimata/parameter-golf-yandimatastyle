...
def eval_val_ttt(
    ...
    # Per-block adaptive LR: lower layers get lower LR (dTTT-style)
    num_blocks = len(base_model.blocks)
    param_groups = []
    assigned = set()
    ttt_min_lr_scale = 0.3
    for bi in range(num_blocks):
        if bi in frozen_block_ids:
            continue
        block = base_model.blocks[bi]
        lr_scale = ttt_min_lr_scale + (1.0 - ttt_min_lr_scale) * (bi / max(num_blocks - 1, 1))
        block_lr = h.ttt_lr * lr_scale
        block_params = [p for p in block.parameters() if p.requires_grad]
        for p in block_params:
            assigned.add(id(p))
        if block_params:
            param_groups.append({"params": block_params, "lr": block_lr})
    other_params = [p for p in ttt_params if id(p) not in assigned]
    if other_params:
        param_groups.append({"params": other_params, "lr": h.ttt_lr})
    optimizer = torch.optim.AdamW(param_groups, betas=(0.9, 0.95), weight_decay=0.01)
    log_fn(f"ttt_sliding:per_block_lr groups={len(param_groups)} lr_range=[{h.ttt_lr*ttt_min_lr_scale:.6f}, {h.ttt_lr:.6f}]")
    batch_seqs = h.ttt_batch_seqs
    t0 = time.perf_counter()

    # Save original weights for per-chunk reset
    if h.ttt_reset_per_chunk:
        _ttt_original_sd = {k: v.detach().clone() for k, v in base_model.state_dict().items()}
    ...
    for ci in range(num_chunks):
        ...
        # Per-chunk weight reset: start fresh from original weights
        if h.ttt_reset_per_chunk and ci > 0:
            base_model.load_state_dict(_ttt_original_sd, strict=True)
            # Reset optimizer state too — rebuild per-block groups
            new_groups = []
            for pg in param_groups:
                new_groups.append({"params": pg["params"], "lr": pg["lr"]})
            optimizer = torch.optim.AdamW(new_groups, betas=(0.9, 0.95), weight_decay=0.01)
        ...
        if not is_last_chunk and h.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = pg.get('_base_lr', pg['lr']) * cos_scale
                    if '_base_lr' not in pg:
                        pg['_base_lr'] = pg['lr'] / max(cos_scale, 1e-8)
