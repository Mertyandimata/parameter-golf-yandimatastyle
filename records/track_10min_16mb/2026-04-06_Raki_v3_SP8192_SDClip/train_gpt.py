...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Catalytic Residual: x + c * f(x), learned per-dim catalyst
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        cat_a = self.catalytic_attn.to(dtype=x_in.dtype)[None, None, :]
        x_out = x_in + cat_a * self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        mlp_out = self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor)
        cat_m = self.catalytic_mlp.to(dtype=x_out.dtype)[None, None, :]
        x_out = x_out + cat_m * self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_out
        return x_out
...
def eval_val_ttt(
    ...
    optimizer = torch.optim.AdamW(ttt_params, lr=h.ttt_lr, betas=(0.9, 0.95), weight_decay=0.01)
